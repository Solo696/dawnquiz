...
function ChallengeScreen({ onExit }) {
  ...
  function handleCopy() {
    try { navigator.clipboard.writeText(challengeUrl); } catch {}
    setCopied(true);
    setTimeout(() => setCopied(false), 2500);
  }

  const shareText = `⚔ DawnQuiz Challenge!
      ...
      <div style={{ padding: "1.25rem 1.25rem 2rem" }}>
        ...
        <div style={{ textAlign: "center", margin: "1.25rem 0 1.75rem" }}>
          ...
          <div style={{ color: T.textMuted, fontSize: "0.82rem", marginTop: "0.35rem", lineHeight: 1.5 }}>
            Generate a link — your friend plays the same questions and their score goes head-to-head with yours.
          </div>
        </div>
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            {["beginner", "intermediate", "expert"].map(d => {
              const dtm = TIER_META[d];
              const isActive = tier === d;
              return (
                <button key={d} onClick={() => { setTier(d); setCopied(false); setSeed(genChallengeSeed()); }} style={{
                  display: "flex", alignItems: "center", gap: "0.65rem",
                  padding: "0.6rem 0.85rem", borderRadius: 12, fontFamily: "inherit", cursor: "pointer",
                  border: `1.5px solid ${isActive ? dtm.color : T.borderHigh}`,
                  background: isActive ? dtm.bg : T.surface, textAlign: "left",
                }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: T.textMuted, fontSize: "0.68rem", marginTop: "0.05rem" }}>
                      {d === "beginner" ? "Core concepts · Protocol basics" :
                       d === "intermediate" ? "DeFi · Token economics · Node mechanics" :
                       "Attack vectors · ZK proofs · Advanced protocol"}
                    </div>
                  </div>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {TIER_LENGTH_OPTIONS.map(n => (
              <button key={n} onClick={() => { setLength(n); setCopied(false); setSeed(genChallengeSeed()); }} style={{
                flex: 1, padding: "0.55rem 0", borderRadius: 10, fontFamily: "inherit", cursor: "pointer",
                border: `1.5px solid ${length === n ? tm.color : T.borderHigh}`,
                background: length === n ? tm.bg : T.surface,
                color: length === n ? tm.color : T.textMuted,
                fontWeight: length === n ? 800 : 500, fontSize: "0.9rem",
              }}>{n}Q</button>
            ))}
          </div>
        </div>
        ...
        {/* Generated link */}
        <div style={{ ...css.card, marginBottom: "1rem", border: `1.5px solid ${tm.color}44`, background: tm.bg }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
            <div style={{ color: tm.color, fontWeight: 800, fontSize: "0.8rem" }}>
              {tm.icon} {tm.label} · {length}Q Challenge Link
            </div>
            <button onClick={regenerate} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: "0.72rem", fontFamily: "inherit" }}>
              🔄 New link
            </button>
          </div>
          <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
            <div style={{
              flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8,
              padding: "0.5rem 0.7rem", fontSize: "0.68rem", color: T.textMuted,
            ...
            <button onClick={handleCopy} style={{
              ...
              background: copied ? T.green + "22" : T.gold + "22",
              border: `1px solid ${copied ? T.green + "66" : T.gold + "66"}`,
              color: copied ? T.green : T.gold,
              cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: "0.75rem",
              transition: "all 0.2s",
            }}>
              {copied ? "✓ Copied!" : "Copy"}
            </button>
          </div>
          <div style={{ color: T.textDim, fontSize: "0.65rem", marginTop: "0.5rem" }}>
            Anyone who opens this link plays the same seeded questions. Each link is unique.
          </div>
        </div>

        {/* Share button */}
        <ShareBtn text={shareText} label="⚔ Share Challenge" style={{ marginBottom: "0.65rem" }} />
        <Btn variant="ghost" onClick={onExit}>← Back to Home</Btn>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const views = {
    ...
    challenge:   <ChallengeScreen onExit={() => setScreen("home")} />,
