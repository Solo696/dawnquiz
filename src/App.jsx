...
import {
  ...
  createChallenge, getChallengeByCode, joinChallenge,
  submitCreatorScore, submitOpponentScore, buildChallengeShareUrl, genChallengeCode,
  logEvent,
  statsTotalVisits, statsUniqueVisitors, statsVisitsPerDay,
  statsTotalRuns, statsRunsByTier, statsDailyAnswers, statsTopPlayers, statsRecentEvents,
} from "./supabase.js";
...
const SFX = {
  ...
  // ── Tournament sounds ──────────────────────────────────────
  tournamentStart() {
    // Battle horn — dramatic low-to-high fanfare
    duckMusic(2200);
    try {
      const ctx = getCtx();
      const master = ctx.createGain();
      master.gain.setValueAtTime(0.0, ctx.currentTime);
      master.gain.linearRampToValueAtTime(0.38, ctx.currentTime + 0.08);
      master.connect(ctx.destination);
      // Two bold low hits
      [[130.81, 0], [130.81, 0.18]].forEach(([freq, delay]) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
        osc.type = "sawtooth";
        osc.frequency.setValueAtTime(freq, ctx.currentTime + delay);
      ...
      // Rising arpeggio: E2 G2 B2 E3 B3 E4
      [[82.41,0.42],[98.00,0.52],[123.47,0.62],[164.81,0.74],[246.94,0.88],[329.63,1.04]].forEach(([freq, delay]) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
        osc.type = delay > 0.85 ? "triangle" : "square";
        osc.frequency.setValueAtTime(freq, ctx.currentTime + delay);
        g.gain.setValueAtTime(0.7, ctx.currentTime + delay);
      ...
      [329.63, 523.25, 659.25, 987.77].forEach((freq, i) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
        osc.type = "triangle";
        osc.frequency.setValueAtTime(freq, ctx.currentTime + 1.22);
        g.gain.setValueAtTime(0.6 - i * 0.1, ctx.currentTime + 1.22);
        g.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 1.22 + 0.7);
        osc.connect(g); g.connect(master);
        osc.start(ctx.currentTime + 1.22); osc.stop(ctx.currentTime + 1.22 + 0.8);
      });
    } catch {}
  },

  tournamentCorrect() {
    // Sharper, punchier correct — competitive edge
    duckMusic(500);
    try {
      const ctx = getCtx();
      const master = ctx.createGain();
      ...
      master.connect(ctx.destination);
      // Quick double-ping: perfect fifth
      [[659.25, 0], [987.77, 0.07], [1318.51, 0.14]].forEach(([freq, delay]) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
        osc.type = "square";
        osc.frequency.setValueAtTime(freq, ctx.currentTime + delay);
  ...
  tournamentTimerWarn() {
    // Urgent double-beep — fires when run timer < 20%
    try {
      const ctx = getCtx();
      const master = ctx.createGain();
  ...
  tournamentSubmit() {
    // Score posted — triumphant rising flourish
    duckMusic(2500);
    try {
      const ctx = getCtx();
      const master = ctx.createGain();
      master.gain.setValueAtTime(0.0, ctx.currentTime);
      master.gain.linearRampToValueAtTime(0.32, ctx.currentTime + 0.06);
      master.connect(ctx.destination);
      // Ascending major scale run
      [523.25, 587.33, 659.25, 783.99, 880, 1046.5, 1174.66, 1318.5].forEach((freq, i) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
      ...
      // Sustained bright chord at end
      [523.25, 659.25, 783.99, 1046.5].forEach((freq, i) => {
        const osc = ctx.createOscillator(); const g = ctx.createGain();
        osc.type = "sine";
        const t0 = ctx.currentTime + 0.65;
        osc.frequency.setValueAtTime(freq, t0);
        g.gain.setValueAtTime(0.5 - i * 0.08, t0);
...
const APP_URL = "https://dawnquiz.vercel.app";

// Generate a stable 4-char hex suffix from a device fingerprint
// Stored in localStorage so it never changes for this device
function getDeviceSuffix() {
  const LS_KEY = "dawn_device_suffix";
  try {
    let sfx = localStorage.getItem(LS_KEY);
    if (!sfx) {
      // Generate from random + timestamp, take 4 hex chars
      sfx = Math.floor(Math.random() * 0xFFFF).toString(16).toUpperCase().padStart(4, "0");
      localStorage.setItem(LS_KEY, sfx);
    }
    return sfx;
  } catch { return "0000"; }
}

// Format username for display: "Name#A9E3"
function formatUsername(username) {
  if (!username) return "Anonymous";
  if (username.includes("#")) return username; // already suffixed
  return `${username}#${getDeviceSuffix()}`;
}
...
function getChallengeCodeFromUrl() {
  try { return new URLSearchParams(window.location.search).get("chl") || null; } catch { return null; }
}
...
const T = {
  bg: "#050d1a",        // Dawn deep navy
  surface: "#0a1628",   // slightly lighter navy
  surfaceHigh: "#0f1f35", // card surface
  border: "#1a3050",    // navy border
  borderHigh: "#2a2a48",
  gold: "#fbbf24",   // kept for sunrays
  orange: "#FF6B1A", // Dawn brand orange
  goldDim: "#92400e",
  text: "#eef2ff",
  textMuted: "#7b90b8",
  textDim: "#2e4a6e",
...
function Btn({ children, onClick, variant = "primary", disabled, style = {}, ...rest }) {
  ...
  const variants = {
    primary: { background: "linear-gradient(135deg,#c44d0e,#e85d15,#FF6B1A)", color: "#fff", boxShadow: "0 4px 24px #FF6B1A33" },
...
function calcMultiplier(meta, correctStreak, isPerfect = false) {
  ...
  // Graduated speed bonus: +10% per 5 seconds saved (earn modes with rewards only)
  // meta.secsPerQ = per-question time allocation; meta.secsUsed = actual time taken
  if (meta?.secsPerQ > 0 && meta?.secsUsed !== undefined && meta?.hasReward) {
    const timeLeft = Math.max(0, meta.secsPerQ - meta.secsUsed);
    const speedTiers = Math.floor(timeLeft / 5);
    if (speedTiers > 0) {
      const speedBonusPct = speedTiers * 0.10;  // +10% per 5s saved
      bonuses.push({ label: `Speed ×${speedTiers} (${timeLeft.toFixed(0)}s left)`, bonus: speedBonusPct, icon: "⚡" });
    }
  }
...
function GameScreen({ player, dispatch, allQuestions, audio, onExit, challengeSeed = null }) {
  ...
  const RUN_DIFFICULTIES = [
    { key: "relaxed",  label: "Relaxed",  icon: "🌿", secsPerQ: 45, color: T.green,  desc: "Plenty of time to think" },
    { key: "standard", label: "Standard", icon: "⚡", secsPerQ: 30, color: T.gold,   desc: "Balanced challenge" },
    { key: "pressure", label: "Pressure", icon: "🔴", secsPerQ: 15, color: T.red,    desc: "Fast — no hesitation" },
  ];
  ...
  const qStartTimeRef = useRef(null);  // timestamp when current question appeared
  ...
  function handleStartRun() {
    ...
    logEvent("run_start", player.username, { tier: start, length: tierLength, difficulty: runDifficulty, practice: practiceMode });
  ...
  function handleAnswer(idx, meta) {
    ...
    setSelected(idx);
    setAnswered(true);
    // Calculate per-question timing for graduated speed bonus
    const secsUsed = qStartTimeRef.current ? (Date.now() - qStartTimeRef.current) / 1000 : 0;
    const activeDiffCfg = RUN_DIFFICULTIES.find(d => d.key === runDifficulty);
    const secsPerQ = activeDiffCfg?.secsPerQ || 0;
    // hasReward: only tiers with sunray rewards count (5Q beginner/intermediate = 0 reward)
    const hasReward = !practiceMode && checkpointSunrays(currentTier || "beginner", tierLength) > 0;
    const enrichedMeta = { ...(meta || {}), secsUsed, secsPerQ, hasReward, hintUsed: meta?.hintUsed || false };
    setAnswerMeta(enrichedMeta);
    const isCorrect = idx === q.correct;
    setRunLog(log => [...log, { qi, diff: currentTier, correct: isCorrect, fast: secsUsed < secsPerQ * 0.5, hintUsed: enrichedMeta.hintUsed }]);
    if (isCorrect) {
  ...
  function completeTier(finalTierSunrays, finalCorrect, finalLog, isPerfectTier) {
    ...
    if (tierQueue.length === 0) {
      ...
      logEvent("run_complete", player.username, {
        tiers: [...completedTiers.map(t => t.tier), currentTier],
        sunrays: tierTotal,
        correct: finalCorrect,
        total: TOTAL,
        perfect: isPerfectTier,
    ...
    return (
      <div style={css.screen}>
        <div style={{ padding: "2rem 1.25rem", textAlign: "center" }}>
          ...
          {/* Share tier result */}
          <div style={{ maxWidth: 300, margin: "0.75rem auto 0" }}>
            <ShareBtn
              text={`${tm.icon} ${tm.label} Complete — DawnQuiz\n\n${tierResult.correct}/${tierResult.total} correct · ${Math.round(tierResult.correct/tierResult.total*100)}% accuracy\n+${tierResult.sunraysEarned} ☀ banked${tierResult.isPerfect ? " · ✦ Perfect!" : ""}\nRank: ${getRank(player.sunrays).name}\n\nChallenge me → ${APP_URL}`}
              label="Share This Tier"
            />
          </div>

          {/* CTA buttons */}
          <div style={{ maxWidth: 300, margin: "0.75rem auto 0", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {!isLastTier && nextTm && (
        ...
        <div style={{ padding: "2.5rem 1.5rem", textAlign: "center" }}>
          ...
          <div style={{ maxWidth: 280, margin: "1.25rem auto 0" }}>
            ...
            {(() => {
              const challengeUrl = buildChallengeUrl(mySeedRef.current, tierLength);
              return (
                <div style={{ ...css.card, marginBottom: "0.75rem", textAlign: "left", padding: "0.85rem 1rem" }}>
                  <div style={{ color: T.text, fontWeight: 800, fontSize: "0.82rem", marginBottom: "0.35rem" }}>⚔ Challenge a Friend</div>
                  <div style={{ color: T.textMuted, fontSize: "0.72rem", marginBottom: "0.6rem", lineHeight: 1.5 }}>
                    They play the exact same questions you just answered. Can they beat your score?
                  </div>
                  <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
                    <div style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "0.45rem 0.65rem", fontSize: "0.68rem", color: T.textMuted, fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {challengeUrl}
                    </div>
                    <button onClick={() => { try { navigator.clipboard.writeText(challengeUrl); } catch {} }}
                      style={{ flexShrink: 0, padding: "0.45rem 0.75rem", borderRadius: 8, background: T.gold + "22", border: `1px solid ${T.gold}66`, color: T.gold, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: "0.72rem" }}>
                      Copy
                    </button>
                  </div>
                </div>
...
function BlackboxChallenge({ player, dispatch, audio, onExit }) {
  const [pool] = useState(() => shuffle(BLACKBOX_QS).slice(0, 10).map(shuffleOptions));
...
function DeployerFinalRun({ player, dispatch, audio, onExit, onVictory }) {
  const [pool] = useState(() => shuffle(DEPLOYER_QS).map(shuffleOptions));
...
function useAdmins() {
  ...
  function addAdmin(name, pin, role = "admin", canAcademy = false) {
          ...
          return (
            <div key={a.id} style={{ ...css.card, display: "flex", alignItems: "center", gap: "0.75rem", borderLeft: `3px solid ${isMasterEntry ? T.gold : T.blue}` }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: T.textMuted, fontSize: "0.7rem", marginTop: "0.1rem" }}>
                  {a.role} · added {a.addedAt}
                  {a.role !== "master" && isMaster && (
                    <span
                      onClick={e => { e.stopPropagation(); setAdmins(prev => prev.map(x => x.id === a.id ? { ...x, canAcademy: !x.canAcademy } : x)); }}
                      style={{ marginLeft: "0.5rem", cursor: "pointer", color: a.canAcademy ? T.gold : T.textDim, fontSize: "0.67rem", fontWeight: 700 }}
                      title="Toggle Academy access">
                      📚 {a.canAcademy ? "Academy ✓" : "Academy ✗"}
                    </span>
                  )}
                </div>
              </div>
...
const HAS_PROXY = (() => {
  try {
    ...
    return host.includes("vercel.app") || host.includes("dawninternet") || host.includes("deployer") || host.includes("dawnquiz");
  } catch { return false; }
})();
...
const BLANK_MODULE = { title: "", icon: "📘", color: "#60a5fa", desc: "", sunrays: 1, lessons: [{ title: "", content: "" }] };
function normLesson(l) { return typeof l === "string" ? { title: l, content: "" } : (l || { title: "", content: "" }); }
const MODULE_ICONS  = ["📘","☀","⬡","◼","⊕","◈","🌐","⚡","🔬","🗺","🧠","🛠","🔗","🌱","🏗"];
...
function AcademyAIKeySetup() {
  ...
  const providers = [
    { id: "gemini",    name: "Gemini",    note: "Free · 1,500 req/day", accent: "#60a5fa" },
    { id: "groq",      name: "Groq",      note: "Free · 25 req/day",    accent: "#a78bfa" },
    { id: "anthropic", name: "Claude",    note: "$5 credit",            accent: "#c4b5fd" },
    { id: "openai",    name: "OpenAI",    note: "Pay-per-use",          accent: "#34d399" },
    { id: "grok",      name: "Grok",      note: "Free tier",            accent: "#e2e8f0" },
  ];
  const cur = providers.find(p => p.id === prov) || providers[0];

  function save() {
    if (!key.trim()) return;
    try {
      localStorage.setItem(`dawn_key_${prov}`, JSON.stringify(key.trim()));
      localStorage.setItem("dawn_active_provider", JSON.stringify(prov));
    } catch {}
    setSaved(true);
  ...
  return (
    <div style={{ ...css.card, border: `1px solid ${T.borderHigh}` }}>
      <div style={{ color: T.text, fontWeight: 700, fontSize: "0.88rem", marginBottom: "0.75rem" }}>🔑 Configure AI Provider</div>
      <div style={{ color: T.textMuted, fontSize: "0.75rem", marginBottom: "0.85rem", lineHeight: 1.5 }}>
        Add a free API key to enable AI module generation. Gemini and Groq are free.
      </div>
      <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap", marginBottom: "0.75rem" }}>
        {providers.map(p => (
          <button key={p.id} onClick={() => { setProv(p.id); setSaved(false); setKey(""); }} style={{ padding: "0.35rem 0.65rem", borderRadius: 8, border: `1px solid ${prov === p.id ? p.accent : T.borderHigh}`, background: prov === p.id ? "#0d0d1a" : T.surface, color: prov === p.id ? p.accent : T.textMuted, cursor: "pointer", fontSize: "0.7rem", fontFamily: "inherit" }}>
            {p.name} <span style={{ opacity: 0.7 }}>({p.note})</span>
          </button>
        ))}
      </div>
      <div style={{ position: "relative", marginBottom: "0.6rem" }}>
        <input
          ...
          placeholder={`Paste your ${cur.name} API key`}
          style={{ ...css.inp, paddingRight: "3rem" }}
        />
        <button onClick={() => setVis(v => !v)} style={{ position: "absolute", right: "0.5rem", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: "0.75rem", fontFamily: "inherit" }}>{vis ? "Hide" : "Show"}</button>
      </div>
      <Btn onClick={save} disabled={!key.trim() || saved} style={{ background: cur.accent, color: "#000" }}>
        {saved ? "✓ Saved — reloading…" : `Save ${cur.name} Key`}
      </Btn>
    </div>
  );
}
...
function AcademyAdmin({ player, dispatch }) {
  ...
  function openEdit(m)    { setMForm({ ...m, lessons: m.lessons.map(normLesson) }); setEditTarget(m); setMErr(""); setModuleView("manual"); }

  function saveModule() {
    if (!mForm.title.trim()) { setMErr("Title is required."); return; }
    const normLessons = mForm.lessons.map(normLesson);
    if (!normLessons.some(l => l.title.trim())) { setMErr("Add at least one lesson title."); return; }
    const cleaned = { ...mForm, title: mForm.title.trim(), desc: mForm.desc.trim(), lessons: normLessons.filter(l => l.title.trim()), id: editTarget ? editTarget.id : Date.now() };
    if (editTarget) {
  ...
  function addLesson()              { setMForm(f => ({ ...f, lessons: [...f.lessons, { title: "", content: "" }] })); }
  function removeLesson(i)          { setMForm(f => ({ ...f, lessons: f.lessons.filter((_, j) => j !== i) })); }
  function updateLessonTitle(i,v)   { setMForm(f => { const ls = f.lessons.map(normLesson); ls[i] = { ...ls[i], title: v }; return { ...f, lessons: ls }; }); }
  function moveLesson(i, dir) {
    setMForm(f => {
      const ls = [...f.lessons.map(normLesson)];
      const target = i + dir;
      if (target < 0 || target >= ls.length) return f;
      [ls[i], ls[target]] = [ls[target], ls[i]];
      return { ...f, lessons: ls };
    });
  }
  function updateLessonContent(i,v) { setMForm(f => { const ls = f.lessons.map(normLesson); ls[i] = { ...ls[i], content: v }; return { ...f, lessons: ls }; }); }
...
CRITICAL Dawn-specific definitions you MUST use — do not contradict these:
- A "node" in Dawn is a device or software instance that shares its unused internet bandwidth with the network and earns Sunray rewards for doing so. It is NOT a blockchain validator or consensus participant.
- "Bandwidth contribution" is the core mechanic — users share spare upload capacity.
- The "BlackBox" is Dawn's dedicated hardware node device requiring 100Mbps+ connection.
- Sunrays are the in-app reward points earned for bandwidth contribution and learning.
- Dawn is a DePIN (Decentralized Physical Infrastructure Network) for internet connectivity.
- Dawn nodes are verified via challenge-response bandwidth proofs, not proof-of-work or proof-of-stake.

Generate ONE learning module about: "${aiTopic.trim()}"

The module should have exactly ${aiCount} lessons. Each lesson has a short progressive title AND 2-4 paragraphs of body content that clearly explains the concept using the Dawn-specific definitions above.
...
Respond with ONLY a valid JSON object — no markdown, no preamble:
{
  ...
  "lessons": [
    {"title": "Lesson 1 title", "content": "3-5 sentences explaining this lesson clearly."},
    {"title": "Lesson 2 title", "content": "3-5 sentences explaining this lesson clearly."}
  ]
}`;
    ...
    try {
      ...
      const normalizedLessons = parsed.lessons.slice(0, 30).map(l => typeof l === "string" ? { title: l, content: "" } : { title: l.title || "", content: l.content || "" });
      setAiGenerated({ ...parsed, id: Date.now(), lessons: normalizedLessons, sunrays: Math.min(3, Math.max(1, Number(parsed.sunrays) || 1)) });
      setAiSelected(true);
    } catch (e) {
        ...
        <>
                  ...
                  <div key={m.id} style={{ ...css.card, borderLeft: `3px solid ${m.color}`, display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ marginTop: "0.4rem", display: "flex", flexWrap: "wrap", gap: "0.25rem" }}>
                        {m.lessons.map((l, i) => (
                          <span key={i} style={{ background: T.surfaceHigh, color: T.textMuted, fontSize: "0.65rem", padding: "2px 6px", borderRadius: 5 }}>{normLesson(l).title}</span>
                        ))}
                      </div>
                    </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                    {mForm.lessons.map(normLesson).map((l, i) => (
                      <div key={i} style={{ background: T.surfaceHigh, borderRadius: 10, padding: "0.5rem 0.6rem" }}>
                        <div style={{ display: "flex", gap: "0.4rem", alignItems: "center", marginBottom: "0.35rem" }}>
                          {/* Reorder arrows */}
                          <div style={{ display: "flex", flexDirection: "column", gap: "1px", flexShrink: 0 }}>
                            <button onClick={() => moveLesson(i, -1)} disabled={i === 0} style={{ background: "none", border: "none", color: i === 0 ? T.textDim : T.textMuted, cursor: i === 0 ? "default" : "pointer", fontSize: "0.6rem", padding: "1px 3px", lineHeight: 1, fontFamily: "inherit" }}>▲</button>
                            <button onClick={() => moveLesson(i, 1)} disabled={i === mForm.lessons.length - 1} style={{ background: "none", border: "none", color: i === mForm.lessons.length - 1 ? T.textDim : T.textMuted, cursor: i === mForm.lessons.length - 1 ? "default" : "pointer", fontSize: "0.6rem", padding: "1px 3px", lineHeight: 1, fontFamily: "inherit" }}>▼</button>
                          </div>
                          <div style={{ width: 20, height: 20, borderRadius: "50%", background: T.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", color: T.textMuted, flexShrink: 0 }}>{i + 1}</div>
                          <input value={l.title} onChange={e => updateLessonTitle(i, e.target.value)} placeholder={`Lesson ${i + 1} title *`} style={{ ...css.inp, flex: 1, marginBottom: 0 }} />
                        ...
                        <textarea value={l.content} onChange={e => updateLessonContent(i, e.target.value)} placeholder="Lesson body — explain the concept clearly (2-5 sentences)" rows={3} style={{ ...css.inp, resize: "vertical", marginBottom: 0, fontSize: "0.78rem" }} />
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {moduleView === "ai" && (
            <div>
              ...
              {/* Provider picker — always visible, shows setup inline if no keys */}
              <div style={{ marginBottom: "0.85rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
                  {Object.values(AI_PROVIDERS).map(p => {
                    ...
                    const isActive = activeProvider === p.id;
                    return (
                      <button key={p.id} onClick={() => { setActiveProvider(p.id); setAiError(""); }}
                        style={{ display: "flex", alignItems: "center", gap: "0.3rem", padding: "0.35rem 0.6rem", borderRadius: 8,
                          border: `1.5px solid ${isActive ? p.accent : T.borderHigh}`,
                          background: isActive ? p.color + "22" : T.surface,
                          cursor: "pointer", fontFamily: "inherit", opacity: hasKey ? 1 : 0.5 }}>
                        <span style={{ fontSize: "0.85rem" }}>{p.icon}</span>
                        <span style={{ color: isActive ? p.accent : T.textMuted, fontWeight: isActive ? 700 : 400, fontSize: "0.72rem" }}>{p.name}</span>
                        {p.free && <span style={{ color: T.green, fontSize: "0.58rem" }}>FREE</span>}
                        {hasKey && <span style={{ color: T.green, fontSize: "0.6rem" }}>✓</span>}
                      </button>
                    );
                  })}
                </div>
                {!hasAnyKey && (
                  <div style={{ marginTop: "0.6rem", padding: "0.65rem 0.85rem", background: "#1c0a0a", border: `1px solid ${T.red}33`, borderRadius: 8, color: T.textMuted, fontSize: "0.75rem", lineHeight: 1.5 }}>
                    No API key saved. Go to <strong style={{ color: T.gold }}>Admin → ❓ Questions → ✨ AI</strong> to add a provider key, then return here.
                  </div>
                )}
              </div>
              {hasAnyKey && (
                <>
                  ...
                  {aiError && (
                    <div style={{ padding: "0.75rem", background: "#2d0a0a", border: `1px solid ${T.red}44`, borderRadius: 10, marginBottom: "0.75rem" }}>
                      <div style={{ color: T.red, fontSize: "0.78rem", marginBottom: "0.5rem" }}>{aiError}</div>
                      <div style={{ color: T.textMuted, fontSize: "0.7rem", marginBottom: "0.4rem" }}>Try a different provider:</div>
                      <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
                        {Object.values(AI_PROVIDERS).filter(p => !!keyValues[p.id] && p.id !== activeProvider).map(p => (
                          <button key={p.id} onClick={() => { setActiveProvider(p.id); setAiError(""); }}
                            style={{ display: "flex", alignItems: "center", gap: "0.3rem", padding: "0.3rem 0.55rem", borderRadius: 7, border: `1px solid ${p.accent}66`, background: p.color + "22", cursor: "pointer", fontFamily: "inherit" }}>
                            <span style={{ fontSize: "0.8rem" }}>{p.icon}</span>
                            <span style={{ color: p.accent, fontSize: "0.7rem", fontWeight: 700 }}>{p.name}</span>
                          </button>
                        ))}
                        {Object.values(AI_PROVIDERS).filter(p => !!keyValues[p.id] && p.id !== activeProvider).length === 0 && (
                          <span style={{ color: T.textDim, fontSize: "0.7rem" }}>No other providers configured.</span>
                        )}
                      </div>
                    </div>
                  )}
                      ...
                      <div style={{ ...css.card, borderLeft: `3px solid ${aiGenerated.color}`, marginBottom: "0.75rem" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                          {aiGenerated.lessons.map((l, i) => {
                            const lesson = normLesson(l);
                            return (
                              <div key={i} style={{ padding: "0.4rem 0.5rem", background: T.surfaceHigh, borderRadius: 7 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                                  <span style={{ width: 20, height: 20, borderRadius: "50%", background: T.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", color: T.textMuted, flexShrink: 0 }}>{i + 1}</span>
                                  <span style={{ fontSize: "0.8rem", color: T.text, fontWeight: 600 }}>{lesson.title}</span>
                                </div>
                                {lesson.content && <div style={{ marginTop: "0.35rem", paddingLeft: "1.6rem" }}>{renderLessonBody(lesson.content)}</div>}
                              </div>
                            );
                          })}
                        </div>
...
// ── Announcements Admin ──────────────────────────────────────
function AnnouncementsAdmin({ announcements, saveAnnouncements }) {
  const [text, setText] = useState("");
  const [error, setError] = useState("");

  function addAnnouncement() {
    if (!text.trim()) { setError("Enter announcement text."); return; }
    const item = { id: Date.now().toString(), text: text.trim(), active: true };
  ...
  return (
    <div>
      <div style={{ color: T.text, fontWeight: 700, fontSize: "0.88rem", marginBottom: "0.75rem" }}>📢 Announcements</div>
      <textarea
        value={text}
        onChange={e => { setText(e.target.value); setError(""); }}
        placeholder="Type an announcement for the Home screen…"
        rows={3}
        style={{ width: "100%", padding: "0.65rem 0.85rem", borderRadius: 10, background: T.surface, border: `1.5px solid ${T.border}`, color: T.text, fontFamily: "inherit", fontSize: "0.82rem", resize: "vertical", outline: "none", boxSizing: "border-box" }}
      />
      {error && <div style={{ color: T.red, fontSize: "0.72rem", margin: "0.25rem 0" }}>{error}</div>}
      <button onClick={addAnnouncement} style={{ marginTop: "0.5rem", padding: "0.55rem 1.1rem", borderRadius: 10, background: T.green + "22", border: `1px solid ${T.green}66`, color: T.green, fontFamily: "inherit", fontWeight: 700, fontSize: "0.82rem", cursor: "pointer" }}>
        + Post Announcement
      </button>
      {announcements.length > 0 && (
        <div style={{ marginTop: "0.85rem", display: "flex", flexDirection: "column", gap: "0.4rem" }}>
          {announcements.map(a => (
            <div key={a.id} style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", padding: "0.55rem 0.75rem", background: T.surface, border: `1px solid ${a.active ? T.green + "55" : T.border}`, borderRadius: 10 }}>
              <div style={{ flex: 1, color: a.active ? T.text : T.textMuted, fontSize: "0.78rem", lineHeight: 1.5 }}>{a.text}</div>
              <div style={{ display: "flex", gap: "0.35rem", flexShrink: 0 }}>
                <button onClick={() => toggle(a.id)} style={{ padding: "0.25rem 0.5rem", borderRadius: 6, border: `1px solid ${a.active ? T.green + "66" : T.border}`, background: "none", color: a.active ? T.green : T.textMuted, fontSize: "0.68rem", cursor: "pointer", fontFamily: "inherit" }}>
                  {a.active ? "Live" : "Off"}
                </button>
                <button onClick={() => remove(a.id)} style={{ padding: "0.25rem 0.5rem", borderRadius: 6, border: `1px solid ${T.red}44`, background: "none", color: T.red, fontSize: "0.68rem", cursor: "pointer", fontFamily: "inherit" }}>✕</button>
              </div>
            </div>
          ))}
        </div>
...
function StatsAdmin() {
  ...
  async function load() {
    ...
    try {
      const [visits, unique, perDay, runs, byTier, daily, top, recent] = await Promise.all([
        statsTotalVisits(),
        statsUniqueVisitors(),
        statsVisitsPerDay(),
        statsTotalRuns(),
        ...
        statsDailyAnswers(),
        statsTopPlayers(),
        statsRecentEvents(15),
      ]);
      setData({ visits, unique, perDay, runs, byTier, daily, top });
      setFeed(recent);
  ...
  const EVENT_LABELS = {
    app_open:      { icon: "👁",  label: "Visit" },
    run_start:     { icon: "🎯", label: "Run started" },
    run_complete:  { icon: "✅", label: "Run completed" },
    daily_answer:  { icon: "🌅", label: "Daily answered" },
  };

  if (loading) return (
    <div style={{ padding: "2rem 0", textAlign: "center", color: T.textMuted, fontSize: "0.85rem" }}>
      <div style={{ marginBottom: "0.5rem" }}>⏳</div>Loading stats…
    </div>
  );
  if (error) return (
    <div style={{ padding: "1rem" }}>
      <div style={{ color: T.red, fontSize: "0.82rem", padding: "0.75rem", background: "#2d0505", borderRadius: 10, marginBottom: "0.75rem", lineHeight: 1.5 }}>
        ⚠ {error}
      </div>
      <div style={{ color: T.textDim, fontSize: "0.72rem", lineHeight: 1.7 }}>
        Make sure:<br/>
        • The <strong style={{color:T.textMuted}}>app_events</strong> table exists in Supabase<br/>
        • RLS policies are set (insert + select)<br/>
        • Your supabase.js keys are correct
      </div>
      <button onClick={load} style={{ marginTop: "0.75rem", padding: "0.5rem 1rem", borderRadius: 8, background: T.surface, border: `1px solid ${T.border}`, color: T.textMuted, cursor: "pointer", fontFamily: "inherit", fontSize: "0.78rem" }}>↻ Retry</button>
    </div>
  );
  if (!data) return null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button onClick={load} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: "0.75rem", fontFamily: "inherit" }}>↻ Refresh</button>
      </div>

      {/* Top stats */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
        {[
          { label: "Total Visits",    value: data.visits,       color: T.blue },
          { label: "Unique Players",  value: data.unique,       color: T.green },
          { label: "Runs Started",    value: data.runs,         color: T.gold },
          { label: "Daily Answered",  value: data.daily.total,  color: "#f97316" },
        ].map(s => (
          <div key={s.label} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem", textAlign: "center" }}>
            <div style={{ color: s.color, fontWeight: 900, fontSize: "1.5rem" }}>{s.value}</div>
            <div style={{ color: T.textDim, fontSize: "0.65rem", marginTop: "0.2rem" }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* Daily quiz accuracy */}
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem" }}>
        <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.5rem" }}>🌅 Daily Quiz Accuracy</div>
        <div style={{ display: "flex", gap: "1rem", justifyContent: "center" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ color: T.green, fontWeight: 800, fontSize: "1.1rem" }}>{data.daily.correct}</div>
            <div style={{ color: T.textDim, fontSize: "0.65rem" }}>Correct</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ color: T.red, fontWeight: 800, fontSize: "1.1rem" }}>{data.daily.incorrect}</div>
            <div style={{ color: T.textDim, fontSize: "0.65rem" }}>Wrong</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ color: T.gold, fontWeight: 800, fontSize: "1.1rem" }}>
              {data.daily.total > 0 ? Math.round(data.daily.correct / data.daily.total * 100) : 0}%
            </div>
      ...
      {/* Runs by tier */}
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem" }}>
        <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.6rem" }}>🎯 Runs by Tier</div>
        {Object.entries(data.byTier).map(([tier, count]) => {
          const tm = TIER_META[tier];
          const total = Object.values(data.byTier).reduce((a, b) => a + b, 0) || 1;
          return (
            <div key={tier} style={{ marginBottom: "0.45rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.72rem", marginBottom: "0.2rem" }}>
                <span style={{ color: tm?.color }}>{tm?.icon} {tm?.label}</span>
                <span style={{ color: T.textMuted }}>{count} runs ({Math.round(count/total*100)}%)</span>
              </div>
              <div style={{ height: 6, background: T.surfaceHigh, borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.round(count/total*100)}%`, background: tm?.color, borderRadius: 3, transition: "width 0.5s" }} />
              </div>
            </div>
          );
        })}
      ...
      {/* Visits last 14 days sparkline */}
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem" }}>
        <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.65rem" }}>📈 Visits — Last 14 Days</div>
        <div style={{ display: "flex", gap: "2px", alignItems: "flex-end", height: 48 }}>
          {data.perDay.map((d, i) => (
            <div key={i} title={`${d.date}: ${d.count} visits`} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
              <div style={{ width: "100%", background: T.blue, borderRadius: "2px 2px 0 0", height: `${Math.max(2, Math.round((d.count / maxDay) * 44))}px`, opacity: d.count === 0 ? 0.15 : 1, transition: "height 0.4s" }} />
            </div>
          ))}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.25rem" }}>
          <span style={{ color: T.textDim, fontSize: "0.6rem" }}>{data.perDay[0]?.date?.slice(5)}</span>
          <span style={{ color: T.textDim, fontSize: "0.6rem" }}>Today</span>
        </div>
      </div>
      ...
      {data.top.length > 0 && (
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem" }}>
          <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.6rem" }}>🏆 Most Active Players</div>
          {data.top.map((p, i) => (
            <div key={p.username} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.35rem 0", borderBottom: i < data.top.length - 1 ? `1px solid ${T.border}` : "none" }}>
              <span style={{ color: i < 3 ? T.gold : T.textDim, fontSize: "0.75rem", minWidth: "1.2rem" }}>#{i+1}</span>
              <span style={{ flex: 1, color: T.text, fontSize: "0.78rem", fontWeight: 600 }}>{p.username}</span>
              <span style={{ color: T.textMuted, fontSize: "0.7rem" }}>{p.runs} runs</span>
              <span style={{ color: T.gold, fontSize: "0.7rem", fontWeight: 700 }}>{p.sunrays} ☀</span>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Recent events feed */}
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "0.75rem" }}>
        <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, marginBottom: "0.6rem" }}>⚡ Recent Activity</div>
        {feed.length === 0 ? (
          <div style={{ color: T.textDim, fontSize: "0.75rem" }}>No events yet.</div>
        ) : feed.map((e, i) => {
          const ev = EVENT_LABELS[e.event] || { icon: "•", label: e.event };
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.3rem 0", borderBottom: i < feed.length - 1 ? `1px solid ${T.border}` : "none" }}>
              <span style={{ fontSize: "0.85rem" }}>{ev.icon}</span>
              <span style={{ flex: 1, color: T.textMuted, fontSize: "0.72rem" }}>
                <span style={{ color: T.text, fontWeight: 600 }}>{e.username || "Anonymous"}</span> · {ev.label}
              </span>
              <span style={{ color: T.textDim, fontSize: "0.65rem" }}>{fmtTime(e.created_at)}</span>
            </div>
          );
        })}
      </div>
...
function AdminPanel({ player, dispatch, onExit, currentAdmin, adminsHook, navLabels, setNavLabels, announcements = [], saveAnnouncements, initialTab = "questions" }) {
    ...
    <div style={css.screen}>
      <div style={{ padding: "1.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", marginBottom: "1.25rem" }}>
          {([["questions","❓ Questions"],["announcements","📢 Announce"],["archive","📋 Archive"],
            ...(currentAdmin?.role === "master" || currentAdmin?.canAcademy ? [["modules","📚 Modules"]] : []),
            ["labels","✏️ Labels"],["reports","⚑ Reports"],["feedback","💬 Feedback"],["admins","👥 Admins"],
            ...(supabaseEnabled ? [["stats","📊 Stats"]] : [])
          ]).map(([k, l]) => (
        ...
        {/* ── Archive tab ── */}
        {tab === "announcements" && <AnnouncementsAdmin announcements={announcements} saveAnnouncements={saveAnnouncements} />}
...
// ── Markdown-lite lesson body renderer ───────────────────────
// Supports: ## Heading, # Heading, **bold**, - bullet, blank line = new para
function renderLessonBody(body) {
  if (!body) return null;
  ...
  function flushPara() {
    ...
    const rendered = parts.map((p, i) =>
      i % 2 === 1 ? <strong key={i} style={{ color: T.text, fontWeight: 700 }}>{p}</strong> : p
    );
    elements.push(
      <p key={elements.length} style={{ margin: "0 0 0.7rem", lineHeight: 1.75, color: T.text }}>{rendered}</p>
    );
    paraLines = [];
  }
  ...
  lines.forEach(line => {
    ...
    if (trimmed.startsWith("## ")) {
      flushPara();
      elements.push(<div key={elements.length} style={{ color: T.gold, fontWeight: 800, fontSize: "0.9rem", margin: "0.85rem 0 0.3rem", borderBottom: `1px solid ${T.border}`, paddingBottom: "0.2rem" }}>{trimmed.slice(3)}</div>);
      return;
    }
    if (trimmed.startsWith("# ")) {
      flushPara();
      elements.push(<div key={elements.length} style={{ color: T.text, fontWeight: 900, fontSize: "0.95rem", margin: "0.85rem 0 0.35rem" }}>{trimmed.slice(2)}</div>);
      return;
    }
    if (trimmed.startsWith("- ") || trimmed.startsWith("• ")) {
      ...
      const rendered = parts.map((p, i) => i % 2 === 1 ? <strong key={i} style={{ fontWeight: 700, color: T.text }}>{p}</strong> : p);
      elements.push(
        <div key={elements.length} style={{ display: "flex", gap: "0.45rem", margin: "0.2rem 0" }}>
          <span style={{ color: T.orange || "#FF6B1A", flexShrink: 0 }}>•</span>
          <span style={{ color: T.text, lineHeight: 1.65 }}>{rendered}</span>
        </div>
      );
      return;
    }
  ...
  flushPara();
  return elements.length ? <div style={{ fontSize: "0.83rem" }}>{elements}</div> : null;
}
      ...
      {lessons.map((l, i) => {
        ...
        return (
          <div key={i} style={{ borderRadius: 9, overflow: "hidden", border: `1px solid ${T.border}` }}>
            <button onClick={() => setOpen(isOpen ? null : i)} style={{ width: "100%", display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.55rem 0.6rem", background: T.surfaceHigh, border: "none", cursor: lesson.content ? "pointer" : "default", fontFamily: "inherit", textAlign: "left" }}>
              ...
              <span style={{ fontSize: "0.83rem", color: T.text, flex: 1 }}>{lesson.title}</span>
              {lesson.content && <span style={{ color: T.textDim, fontSize: "0.7rem" }}>{isOpen ? "▲" : "▼"}</span>}
            </button>
            {isOpen && lesson.content && (
              <div style={{ padding: "0.75rem 0.9rem", background: "#0d0e20", borderTop: `1px solid ${T.border}` }}>
...
function LearningHub({ player, dispatch, onExit, onAdmin }) {
      ...
      <div style={css.screen}>
        <div style={{ padding: "1rem 1.25rem" }}>
          ...
          <div style={{ ...css.card, marginBottom: "1.25rem" }}>
            ...
            <LessonList lessons={active.lessons} />
          </div>
          {done ? (
    ...
    <div style={css.screen}>
      <div style={{ padding: "1.25rem" }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: "0.25rem" }}>
          <div style={{ flex: 1 }}><BackBtn onClick={onExit} /></div>
          {onAdmin && (
            <button onClick={() => onAdmin("modules")} style={{ background: "none", border: `1px solid ${T.borderHigh}`, borderRadius: 8, padding: "0.3rem 0.65rem", color: T.textMuted, cursor: "pointer", fontSize: "0.7rem", fontFamily: "inherit" }}>
              ⚙ Manage Modules
            </button>
          )}
        </div>
        ...
        {onAdmin && (
          <div style={{ marginTop: "1.5rem", padding: "0.75rem 1rem", background: "#0d1020", border: `1px solid ${T.border}`, borderRadius: 12, fontSize: "0.75rem", color: T.textDim, lineHeight: 1.5 }}>
            💡 To add, edit or remove modules, tap <strong style={{ color: T.textMuted }}>⚙ Manage Modules</strong> above. You'll need your admin PIN.
          </div>
        )}
      </div>
    </div>
...
function TournamentScreen({ player, dispatch, onExit, initialCode = null, onCodeConsumed }) {
  ...
  const username = formatUsername(player.username);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setLeaderboard([]);
      setMyBest(null);
      // Stay on "create" tab — success card renders; user taps "View Lobby" to proceed
  ...
  function exportLeaderboardCSV() {
    ...
    const headers = ["Rank","Username","Score","Accuracy","Correct","Total","Time Bonus"];
    ...
    const a    = document.createElement("a");
    a.href = url;
    a.download = `${activeTour?.title || "tournament"}_leaderboard.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  function handleRunComplete(result) {
    ...
    if (accuracy >= 0.7) {
      if (_sfxEnabled) SFX.victory();
    } else {
      if (_sfxEnabled) SFX.wrong();
    }
  }
  ...
  async function handleSubmitScore() {
    ...
    try {
      ...
      setSubmitted(true);
      if (_sfxEnabled) SFX.tournamentSubmit();
        ...
        <div style={{ padding: "2rem 1.25rem", textAlign: "center" }}>
          ...
          {!submitted ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem" }}>
              <div style={{ color: T.green, fontWeight: 700, fontSize: "0.9rem" }}>✅ Score submitted!</div>
              {leaderboard.length > 0 && (
                <button onClick={exportLeaderboardCSV} style={{ background: "none", border: `1px solid ${T.blue}55`, borderRadius: 8, padding: "0.35rem 0.85rem", color: T.blue, cursor: "pointer", fontSize: "0.75rem", fontFamily: "inherit" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", maxWidth: 260, margin: "1rem auto 0" }}>
            ...
            <Btn variant="ghost" onClick={() => { setActiveTour(null); setPhase("lobby"); setRunResult(null); setSubmitted(false); setTab("create"); }}>
          ...
          <div>
            ...
            {!activeTour && (
              <>
                ...
                {(() => {
                  const now = new Date();
                  const live    = browseList.filter(t => now >= new Date(t.starts_at) && now <= new Date(t.ends_at));
                  const ended   = browseList.filter(t => now > new Date(t.ends_at));
                  const renderCard = (t) => {
                    const isLiveCard = now >= new Date(t.starts_at) && now <= new Date(t.ends_at);
                    const isEndedCard = now > new Date(t.ends_at);
                    return (
                      <button key={t.id} onClick={() => handleJoin(t.code)} style={{
                        width: "100%", ...css.card, marginBottom: "0.5rem", textAlign: "left",
                        cursor: "pointer", fontFamily: "inherit",
                        border: `1px solid ${isLiveCard ? T.gold + "66" : T.border}`,
                        background: isLiveCard ? "linear-gradient(135deg,#0d1200,#1a1e00)" : T.surface,
                        opacity: isEndedCard ? 0.7 : 1,
                      }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                          <span style={{ fontSize: "1rem" }}>{isLiveCard ? "🟢" : isEndedCard ? "⬛" : "🕐"}</span>
                          <div style={{ color: isLiveCard ? T.gold : T.text, fontWeight: 700, fontSize: "0.85rem", flex: 1 }}>{t.title}</div>
                          <div style={{ color: isLiveCard ? T.gold : T.textDim, fontSize: "0.68rem", fontWeight: 700 }}>{isLiveCard ? timeLeft(t.ends_at) : isEndedCard ? "Ended" : "Soon"}</div>
                        </div>
                        <div style={{ color: T.textMuted, fontSize: "0.68rem" }}>
                          {TIER_META[t.tier]?.icon} {TIER_META[t.tier]?.label} · {t.tier_length}Q · <span style={{ color: T.blue, fontWeight: 700 }}>{t.code}</span>
                        </div>
                      </button>
                    );
                  };
                  return (
                    <>
                      {live.length > 0 && (
                        <>
                          <div style={{ color: T.green, fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.1em", marginBottom: "0.4rem" }}>🟢 LIVE NOW</div>
                          {live.map(renderCard)}
                        </>
                      )}
                      {ended.length > 0 && (
                        <>
                          <div style={{ color: T.textDim, fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.1em", margin: `${live.length > 0 ? "0.85rem" : 0} 0 0.4rem` }}>⬛ ENDED</div>
                          {ended.map(renderCard)}
                        </>
                      )}
                      {live.length === 0 && ended.length === 0 && browseList.length > 0 && browseList.map(renderCard)}
                    </>
                  );
                })()}
              </>
...
function TournamentRun({ tournament: t, player, onComplete, onExit }) {
  ...
  // Build seeded pool ONCE — useState initializer runs only on mount.
  // If this were a plain const, every setState (timer tick, answer) would
  // re-run shuffleOptions, shifting q.correct and causing wrong-answer bugs.
  const [pool] = useState(() => {
  ...
  const RUN_DIFFICULTIES_T = [
    { key: "relaxed",  secsPerQ: 45 },
    { key: "standard", secsPerQ: 30 },
    { key: "pressure", secsPerQ: 15 },
  ];
  ...
  // Track whether timer warn has fired (only once per run)
  const warnFiredRef = useRef(false);

  useEffect(() => {
    // Play start fanfare on mount
    if (_sfxEnabled) SFX.tournamentStart();
    timerRef.current = setInterval(() => {
      setRunTimeLeft(t => {
        const newT = t <= 1 ? 0 : t - 1;
        // Fire timer warning once when crossing below 20% remaining
        if (!warnFiredRef.current && newT > 0 && newT / totalSecs < 0.2) {
          warnFiredRef.current = true;
          if (_sfxEnabled) SFX.tournamentTimerWarn();
        }
        if (newT === 0) clearInterval(timerRef.current);
        return newT;
      });
    }, 1000);
  ...
  function handleAnswer(i) {
    ...
    if (isCorrect) {
      setCorrect(c => c + 1);
      setTimeBonusTotal(tb => tb + bonus);
      if (_sfxEnabled) SFX.tournamentCorrect();
    } else {
      if (_sfxEnabled) SFX.wrong();
    }
  }
...
function Leaderboard({ player, onExit }) {
  ...
  const you = {
    name: formatUsername(player.username) || "You",
    sunrays: player.sunrays,
    rank: getRank(player.sunrays).name,
...
function DailyQuestion({ player, dispatch, audio, onExit }) {
  ...
  const alreadyDone = (player.dailyHistory || []).includes(todayStr);

  // Stable question — memoized per day so re-renders never re-shuffle options
  const dq = useMemo(() => {
    if (!allQs.length) return null;
    const shuffled = seededShuffle(allQs, 0xDA1A);
  ...
  // 30s countdown — fires only when question is unanswered and not already done today
  const timerActive = !answered && !alreadyDone;
  const onExpire = useCallback(() => {
    if (!answered) {
      setAnswered(true);
      setTimeUp(true);
      audio?.play("wrong");
      // Save result so locked screen can share it
      try {
        localStorage.setItem("dawn_last_daily_result", JSON.stringify({
  ...
  }, [answered, audio, dq, player]);
  const dailyTimeLeft = useCountdown(DAILY_SECS, timerActive, onExpire);

  // Next-day countdown — seconds until next midnight (local)
  const [nextDaySecs, setNextDaySecs] = useState(() => {
    const now = Date.now();
    return Math.max(0, Math.floor(((Math.floor(now / 86400000) + 1) * 86400000 - now) / 1000));
  });
  useEffect(() => {
    if (!alreadyDone && !answered) return;
    const id = setInterval(() => {
      const now = Date.now();
      setNextDaySecs(Math.max(0, Math.floor(((Math.floor(now / 86400000) + 1) * 86400000 - now) / 1000)));
    }, 1000);
  ...
  function handleAnswer(i) {
    ...
    setSel(i);
    setAnswered(true);
    const wasCorrect = i === dq.correct;
    // Persist result so the locked screen can show a share button tomorrow
    try {
      localStorage.setItem("dawn_last_daily_result", JSON.stringify({
        question: dq.question,
        wasCorrect,
        earned: wasCorrect ? 1 + bonus : 0,
        rank: getRank(player.sunrays).name,
        sunrays: player.sunrays,
        date: todayStr,
      }));
    } catch {}
    logEvent("daily_answer", player.username, { correct: wasCorrect, streak: player.currentStreak });
    if (wasCorrect) {
      audio?.play("correct");
  ...
  // ── Already answered today — locked screen with next-day countdown ──
  if (alreadyDone) {
    // Load last result for share text
    const lastResult = (() => {
      try { return JSON.parse(localStorage.getItem("dawn_last_daily_result") || "null"); } catch { return null; }
    })();
    const doneShareText = lastResult?.wasCorrect
      ? `🌅 I got today's Dawn daily question!\n\n"${lastResult.question}"\n\n✓ Correct! +${lastResult.earned} ☀\nI'm a ${lastResult.rank} with ${lastResult.sunrays} Sunrays\n\nCan you get it right? → ${APP_URL}`
      : lastResult
        ? `🌅 Today's Dawn daily question got me! 😅\n\n"${lastResult.question}"\n\nI'm a ${lastResult.rank} · ${lastResult.sunrays} ☀\n\nCan you do better? → ${APP_URL}`
        : `🌅 I completed today's Dawn daily question!\nStreak: ${player.currentStreak} days 🔥\n\nJoin DawnQuiz → ${APP_URL}`;
      ...
      <div style={css.screen}>
        <div style={{ padding: "1.25rem" }}>
          ...
          <div style={{ textAlign: "center", margin: "1.25rem 0" }}>
            ...
            <SectionTitle sub={"Day " + player.currentStreak + " streak 🔥"}>Daily Question</SectionTitle>
          </div>
          <StreakCalendar />
          <div style={{ ...css.card, textAlign: "center", padding: "2rem 1rem", border: `1.5px solid ${T.gold}44`, background: "linear-gradient(135deg,#0d0e1a,#111200)" }}>
            <div style={{ fontSize: "2.5rem", marginBottom: "0.75rem" }}>✅</div>
            <div style={{ color: T.gold, fontWeight: 900, fontSize: "1.1rem", marginBottom: "0.35rem" }}>Completed today!</div>
            <div style={{ color: T.textMuted, fontSize: "0.8rem", marginBottom: "1.5rem" }}>Come back tomorrow for a new question.</div>
            <div style={{ color: T.textDim, fontSize: "0.68rem", marginBottom: "0.3rem" }}>Next question in</div>
            <div style={{ fontVariantNumeric: "tabular-nums", fontWeight: 900, fontSize: "2.2rem", color: T.text, letterSpacing: "0.05em" }}>
              {fmtCountdown(nextDaySecs)}
            </div>
            <div style={{ color: T.textDim, fontSize: "0.6rem", marginTop: "0.2rem", letterSpacing: "0.15em" }}>HH : MM : SS</div>
          </div>
          <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
            <ShareBtn text={doneShareText} label="Share Today's Result" />
            <Btn variant="ghost" onClick={onExit}>← Back to Home</Btn>
          </div>
        </div>
      </div>
    );
  ...
  // ── Active question ───────────────────────────────────────────
  const correct = !timeUp && sel === dq.correct;
...
✓ Correct! +${1 + bonus} ☀
I'm a ${getRank(player.sunrays).name} with ${player.sunrays} Sunrays
...
${timeUp ? "⏱ Time ran out!" : "✗ I got it wrong"} · I'm a ${getRank(player.sunrays).name} · ${player.sunrays} ☀
  ...
  return (
    <div style={css.screen}>
      <div style={{ padding: "1.25rem" }}>
        ...
        {/* 30s countdown bar — always visible, freezes when answered */}
        <div style={{ marginBottom: "0.75rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.72rem", marginBottom: "0.3rem" }}>
            <span style={{ color: T.textMuted }}>⏱ {answered ? "Time used" : "Time remaining"}</span>
            <span style={{ fontWeight: 800, fontVariantNumeric: "tabular-nums",
              color: answered ? T.textMuted : dailyTimeLeft > 10 ? T.green : dailyTimeLeft > 5 ? T.gold : T.red }}>
              {answered ? "—" : `${dailyTimeLeft}s`}
            </span>
          </div>
          <ProgressBar
            value={answered ? 0 : dailyTimeLeft}
            max={DAILY_SECS}
            color={answered ? T.border : dailyTimeLeft > 10 ? T.green : dailyTimeLeft > 5 ? T.gold : T.red}
            height={4}
          />
        </div>
        ...
        {answered && (
          <div style={{ marginTop: "0.75rem", borderRadius: 12, overflow: "hidden", fontSize: "0.82rem", lineHeight: 1.6 }}>
            <div style={{ padding: "0.5rem 1rem", background: correct ? "#022b14" : "#2b0808", borderBottom: `1px solid ${correct ? T.green + "33" : T.red + "33"}`, color: correct ? T.green : T.red, fontWeight: 700 }}>
              {timeUp
                ? `⏱ Time's up! — correct answer: ${dq.options[dq.correct]}`
                : correct
                  ? `✓ Correct! +${1 + bonus} ☀`
                  : `✗ Wrong — correct answer: ${dq.options[dq.correct]}`}
            </div>
        ...
        {/* Next-day countdown after answering */}
        {answered && (
          <div style={{ ...css.card, marginTop: "0.85rem", textAlign: "center", padding: "0.75rem", background: "linear-gradient(135deg,#0d0e1a,#111200)" }}>
            <div style={{ color: T.textDim, fontSize: "0.68rem", marginBottom: "0.2rem" }}>Next question in</div>
            <div style={{ fontVariantNumeric: "tabular-nums", fontWeight: 900, fontSize: "1.5rem", color: T.text, letterSpacing: "0.04em" }}>
              {fmtCountdown(nextDaySecs)}
            </div>
          </div>
        )}
        ...
        {answered && (
          <div style={{ marginTop: "0.85rem", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
            <ShareBtn text={shareText} label="Share" />
...
// ─────────────────────────────────────────────────────────────
//  CHALLENGE A FRIEND SCREEN
// ─────────────────────────────────────────────────────────────
function ChallengeScreen({ onExit, incomingCode = null, onCodeConsumed, player }) {
  // ── Creator state ───────────────────────────────────────────
  const [tier,       setTier]       = useState("expert");
  const [length,     setLength]     = useState(10);
  const [seed,       setSeed]       = useState(() => genChallengeSeed());
  const [copied,     setCopied]     = useState(false);

  // ── Supabase challenge tracking ─────────────────────────────
  const [challenge,  setChallenge]  = useState(null);  // db row
  const [phase,      setPhase]      = useState(incomingCode ? "loading" : "setup");
  // phases: setup | creating | waiting | joined | playing | result | expired | error
  const [error,      setError]      = useState("");
  const [secsLeft,   setSecsLeft]   = useState(0);
  const [isCreator,  setIsCreator]  = useState(!incomingCode);
  const pollRef      = useRef(null);
  const expiryRef    = useRef(null);

  const username = player?.username ? formatUsername(player.username) : "Anonymous";

  // ── Auto-load incoming challenge link ───────────────────────
  useEffect(() => {
    if (incomingCode) {
      loadIncoming(incomingCode);
      onCodeConsumed?.();
    }
    return () => { clearInterval(pollRef.current); clearInterval(expiryRef.current); };
  }, []);

  async function loadIncoming(code) {
    setPhase("loading"); setError("");
    try {
      if (!supabaseEnabled) {
        // No Supabase — just play the seed from URL params
        setPhase("playing");
        return;
      }
      const ch = await getChallengeByCode(code);
      if (!ch) { setError("Challenge not found or already expired."); setPhase("error"); return; }
      if (ch.status === "expired" || new Date() > new Date(ch.expires_at)) {
        setError("This challenge link has expired."); setPhase("expired"); return; }
      if (ch.status === "completed") {
        setError("This challenge has already been completed."); setPhase("expired"); return; }
      // Mark as joined
      await joinChallenge(ch.id, username);
      setChallenge({ ...ch, status: "joined", opponent_name: username });
      setIsCreator(false);
      setPhase("playing");
    } catch(e) { setError(e.message || "Could not load challenge."); setPhase("error"); }
  }

  // ── Creator creates challenge in Supabase ───────────────────
  async function handleCreate() {
    if (!supabaseEnabled) {
      // No Supabase — just show the basic link without waiting room
      setPhase("waiting_nosb");
      return;
    }
    setPhase("creating"); setError("");
    try {
      const ch = await createChallenge({ seed, tier, tierLength: length, createdBy: username, expiryHours: 24 });
      if (!ch) throw new Error("Could not create challenge.");
      setChallenge(ch);
      setPhase("waiting");
      startExpiry(ch.expires_at);
      startPolling(ch.id);
    } catch(e) { setError(e.message || "Could not create challenge."); setPhase("setup"); }
  }

  // ── Poll for status changes (creator waiting for opponent) ──
  function startPolling(id) {
    clearInterval(pollRef.current);
    pollRef.current = setInterval(async () => {
      try {
        const ch = await getChallengeByCode(challenge?.code || "");
        // re-fetch by id more reliably
        const rows = await fetch(`${''/*SUPABASE_URL injected via proxy*/}`)
          .catch(() => null);
        // Use getChallengeByCode since we have the code
        if (ch) {
          setChallenge(ch);
          if (ch.status === "joined") {
            clearInterval(pollRef.current);
            audio_noop();
            setPhase("opponent_joined");
            // Auto-advance after 2s so creator can see the notification
          ...
          if (new Date() > new Date(ch.expires_at)) {
            clearInterval(pollRef.current);
  ...
  function audio_noop() {} // placeholder — no audio dep here
  ...
  function startExpiry(expiresAt) {
    ...
    function tick() {
      ...
      setSecsLeft(secs);
      if (secs === 0) { clearInterval(expiryRef.current); setPhase("expired"); }
    }
    tick();
  ...
  const challengeUrl = challenge
    ? (supabaseEnabled ? buildChallengeShareUrl(challenge.code, challenge.seed, challenge.tier_length) : buildChallengeUrl(seed, length))
    : buildChallengeUrl(seed, length);
    ...
    return (
      <div style={css.screen}>
        <div style={{ padding: "3rem 1.25rem", textAlign: "center" }}>
          ...
          <div style={{ color: T.text, fontWeight: 700 }}>Loading challenge…</div>
        </div>
      </div>
    );
  }
      ...
      <div style={css.screen}>
        <div style={{ padding: "1.25rem" }}>
          ...
          <div style={{ textAlign: "center", margin: "2rem 0" }}>
            <div style={{ fontSize: "2.5rem", marginBottom: "0.75rem" }}>{phase === "expired" ? "⌛" : "⚠️"}</div>
            <div style={{ color: T.red, fontWeight: 900, fontSize: "1.1rem", marginBottom: "0.5rem" }}>
              {phase === "expired" ? "Link Expired" : "Challenge Error"}
            </div>
            <div style={{ color: T.textMuted, fontSize: "0.85rem", lineHeight: 1.6, maxWidth: 280, margin: "0 auto 1.5rem" }}>{error}</div>
            <Btn onClick={handleReset}>Create New Challenge</Btn>
          </div>
        </div>
      </div>
    );
  ...
  // ── Waiting — no Supabase ────────────────────────────────────
  if (phase === "waiting_nosb") {
    const url = buildChallengeUrl(seed, length);
    return (
      <div style={css.screen}>
        <div style={{ padding: "1.25rem" }}>
          <BackBtn onClick={handleReset} />
          <div style={{ textAlign: "center", margin: "1.5rem 0 1.25rem" }}>
            <div style={{ fontSize: "2rem" }}>⚔</div>
            <div style={{ color: T.text, fontWeight: 900, fontSize: "1.3rem", marginTop: "0.5rem" }}>Challenge Ready</div>
            <div style={{ color: T.textMuted, fontSize: "0.78rem", marginTop: "0.3rem" }}>Share this link with your friend</div>
          </div>
          <div style={{ ...css.card, border: `1.5px solid ${T.gold}44`, marginBottom: "0.85rem" }}>
            <div style={{ color: T.textMuted, fontSize: "0.68rem", marginBottom: "0.4rem" }}>CHALLENGE LINK</div>
            <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
              <div style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "0.5rem 0.7rem", fontSize: "0.68rem", color: T.textMuted, fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{url}</div>
              <button onClick={() => handleCopy(url)} style={{ flexShrink: 0, padding: "0.5rem 0.85rem", borderRadius: 8, background: copied ? T.green + "22" : T.gold + "22", border: `1px solid ${copied ? T.green + "66" : T.gold + "66"}`, color: copied ? T.green : T.gold, cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: "0.75rem" }}>
                {copied ? "✓ Copied!" : "Copy"}
              </button>
            </div>
            <div style={{ color: T.textDim, fontSize: "0.65rem", marginTop: "0.45rem" }}>Note: Live waiting room requires Supabase to be configured.</div>
          </div>
          <ShareBtn text={`⚔ DawnQuiz Challenge!

I challenge you to beat me on this ${length}Q ${tm.label} quiz.

${url}

Do you have what it takes? 🌅`} label="Share Challenge" style={{ marginBottom: "0.65rem" }} />
          <Btn variant="ghost" onClick={handleReset}>← New Challenge</Btn>
        </div>
      </div>
    );
  }

  // ── Waiting — with Supabase ───────────────────────────────────
          ...
          <div style={{ textAlign: "center", margin: "1.5rem 0 1.25rem" }}>
            ...
            <div style={{ position: "relative", width: 80, height: 80, margin: "0 auto 1rem" }}>
              <div style={{
                ...
                animation: "challengePulse 1.8s ease-in-out infinite",
              }} />
              <div style={{
                position: "absolute", inset: 8, borderRadius: "50%",
                border: `2px solid ${T.gold}66`,
                animation: "challengePulse 1.8s ease-in-out infinite 0.4s",
              }} />
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem" }}>⚔</div>
            </div>
            <div style={{ color: T.text, fontWeight: 900, fontSize: "1.2rem" }}>Waiting for opponent…</div>
            <div style={{ color: T.textMuted, fontSize: "0.78rem", marginTop: "0.3rem" }}>
              Share the link below · Link expires in{" "}
              <span style={{ color: secsLeft < 3600 ? T.red : T.gold, fontWeight: 700 }}>{fmtExpiry(secsLeft)}</span>
            </div>
          </div>

          {/* Challenge details */}
          <div style={{ ...css.card, marginBottom: "0.85rem", border: `1px solid ${T.gold}33` }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.6rem" }}>
              <div style={{ color: T.textMuted, fontSize: "0.72rem" }}>Your challenge</div>
              <div style={{ display: "flex", gap: "0.4rem" }}>
                <span style={{ ...css.tag(tm.color) }}>{tm.icon} {tm.label}</span>
                <span style={{ ...css.tag(T.textMuted) }}>{challenge.tier_length}Q</span>
              </div>
            </div>
            <div style={{ color: T.textMuted, fontSize: "0.68rem", marginBottom: "0.35rem" }}>CHALLENGE LINK</div>
            <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
              <div style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "0.5rem 0.7rem", fontSize: "0.68rem", color: T.textMuted, fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{challengeUrl}</div>
              <button onClick={() => handleCopy(challengeUrl)} style={{ flexShrink: 0, padding: "0.5rem 0.85rem", borderRadius: 8, background: copied ? T.green + "22" : T.gold + "22", border: `1px solid ${copied ? T.green + "66" : T.gold + "66"}`, color: copied ? T.green : T.gold, cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: "0.75rem" }}>
                {copied ? "✓" : "Copy"}
              </button>
            </div>
          </div>
...
I challenge you to beat me on this ${challenge.tier_length}Q ${TIER_META[challenge.tier]?.label || ""} quiz.

${challengeUrl}

Do you have what it takes? 🌅`} label="Share Challenge" style={{ marginBottom: "0.65rem" }} />
          <Btn variant="ghost" onClick={handleReset} style={{ color: T.textMuted, borderColor: T.border }}>← Cancel</Btn>
        </div>
        <style>{`
  ...
  // ── Opponent just joined — flash notification ─────────────────
  if (phase === "opponent_joined" && challenge) {
    return (
      <div style={css.screen}>
        <div style={{ padding: "2rem 1.25rem", textAlign: "center" }}>
          <div style={{ fontSize: "3rem", marginBottom: "0.75rem" }}>🔥</div>
          <div style={{ color: T.green, fontWeight: 900, fontSize: "1.3rem" }}>Opponent Joined!</div>
          <div style={{ color: T.textMuted, fontSize: "0.88rem", marginTop: "0.4rem" }}>
            <span style={{ color: T.text, fontWeight: 700 }}>{challenge.opponent_name || "Your friend"}</span> accepted the challenge
          </div>
          <div style={{ color: T.textDim, fontSize: "0.78rem", marginTop: "0.75rem" }}>Starting game…</div>
        </div>
      </div>
    );
  }
  ...
  if (phase === "result" && challenge) {
    const creatorWon = (challenge.creator_score || 0) >= (challenge.opponent_score || 0);
    const iAmCreator = isCreator;
    const myScore    = iAmCreator ? challenge.creator_score  : challenge.opponent_score;
    const theirScore = iAmCreator ? challenge.opponent_score : challenge.creator_score;
    const myCorrect  = iAmCreator ? challenge.creator_correct  : challenge.opponent_correct;
    const theirCorrect = iAmCreator ? challenge.opponent_correct : challenge.creator_correct;
    const total      = challenge.tier_length;
    const won        = iAmCreator ? creatorWon : !creatorWon;
    return (
      <div style={css.screen}>
        <div style={{ padding: "2rem 1.25rem", textAlign: "center" }}>
          <div style={{ fontSize: "3rem", marginBottom: "0.75rem" }}>{won ? "🏆" : "😅"}</div>
          <div style={{ color: won ? T.gold : T.textMuted, fontWeight: 900, fontSize: "1.4rem" }}>{won ? "You Won!" : "You Lost"}</div>
          <div style={{ ...css.card, maxWidth: 300, margin: "1.5rem auto", border: `1px solid ${won ? T.gold + "44" : T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-around", marginBottom: "0.75rem" }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ color: T.gold, fontWeight: 900, fontSize: "1.5rem" }}>{myScore ?? "—"}</div>
                <div style={{ color: T.textDim, fontSize: "0.65rem" }}>YOUR SCORE</div>
                <div style={{ color: T.textMuted, fontSize: "0.72rem" }}>{myCorrect}/{total} correct</div>
              </div>
              <div style={{ color: T.border, display: "flex", alignItems: "center" }}>vs</div>
              <div style={{ textAlign: "center" }}>
                <div style={{ color: T.text, fontWeight: 900, fontSize: "1.5rem" }}>{theirScore ?? "—"}</div>
                <div style={{ color: T.textDim, fontSize: "0.65rem" }}>{iAmCreator ? (challenge.opponent_name || "FRIEND") : "YOU SET"}</div>
                <div style={{ color: T.textMuted, fontSize: "0.72rem" }}>{theirCorrect}/{total} correct</div>
              </div>
            </div>
          </div>
          <div style={{ maxWidth: 280, margin: "0 auto" }}>
            <Btn onClick={handleReset} style={{ marginBottom: "0.5rem" }}>New Challenge</Btn>
            <Btn variant="ghost" onClick={onExit}>← Home</Btn>
          </div>
        </div>
      </div>
    );
    ...
    <div style={css.screen}>
      <div style={{ padding: "1.25rem 1.25rem 2rem" }}>
        ...
        <div style={{ textAlign: "center", margin: "1.25rem 0 1.75rem" }}>
          ...
          <div style={{ color: T.text, fontWeight: 900, fontSize: "1.4rem" }}>Challenge a Friend</div>
          <div style={{ color: T.textMuted, fontSize: "0.82rem", marginTop: "0.35rem", lineHeight: 1.5 }}>
            Generate a link — your friend plays the same questions. First to the higher score wins.
          </div>
        </div>

        {/* Tier picker */}
        <div style={{ marginBottom: "1rem" }}>
          <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.1em", marginBottom: "0.45rem" }}>STARTING TIER</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            {["beginner", "intermediate", "expert"].map(d => {
              const dtm = TIER_META[d]; const isActive = tier === d;
              return (
                <button key={d} onClick={() => { setTier(d); setCopied(false); }} style={{ display: "flex", alignItems: "center", gap: "0.65rem", padding: "0.6rem 0.85rem", borderRadius: 12, fontFamily: "inherit", cursor: "pointer", border: `1.5px solid ${isActive ? dtm.color : T.borderHigh}`, background: isActive ? dtm.bg : T.surface, textAlign: "left" }}>
                  <span style={{ fontSize: "1.2rem" }}>{dtm.icon}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: isActive ? dtm.color : T.text, fontWeight: 700, fontSize: "0.85rem" }}>{dtm.label}</div>
                    <div style={{ color: T.textMuted, fontSize: "0.68rem" }}>
                      {d === "beginner" ? "Core concepts · Protocol basics" : d === "intermediate" ? "DeFi · Token economics · Node mechanics" : "Attack vectors · ZK proofs · Advanced protocol"}
                    </div>
                  </div>
                  {isActive && <span style={{ color: dtm.color }}>✓</span>}
                </button>
              );
            })}
          </div>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          <div style={{ color: T.textMuted, fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.1em", marginBottom: "0.45rem" }}>QUESTIONS</div>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {TIER_LENGTH_OPTIONS.map(n => (
              <button key={n} onClick={() => setLength(n)} style={{ flex: 1, padding: "0.55rem 0", borderRadius: 10, fontFamily: "inherit", cursor: "pointer", border: `1.5px solid ${length === n ? tm.color : T.borderHigh}`, background: length === n ? tm.bg : T.surface, color: length === n ? tm.color : T.textMuted, fontWeight: length === n ? 800 : 500, fontSize: "0.9rem" }}>{n}Q</button>
            ))}
          </div>
        </div>

        {error && <div style={{ color: T.red, fontSize: "0.78rem", marginBottom: "0.75rem", padding: "0.5rem 0.75rem", background: "#2d0505", borderRadius: 8 }}>{error}</div>}

        <Btn onClick={handleCreate} disabled={phase === "creating"}>
          {phase === "creating" ? "Creating…" : "⚔ Generate Challenge Link"}
        </Btn>

        {!supabaseEnabled && (
          <div style={{ color: T.textDim, fontSize: "0.7rem", textAlign: "center", marginTop: "0.75rem", lineHeight: 1.5 }}>
            💡 Connect Supabase to enable the live waiting room — opponents joining, expiry countdown, and results.
          </div>
        )}
      </div>
    </div>
...
//  ABOUT SCREEN
// ─────────────────────────────────────────────────────────────
function AboutScreen({ onClose, onNav }) {
  const DAWN_LINKS = [
    { icon: "🌐", label: "Dawn Website",      url: "https://www.dawninternet.com",               color: T.blue },
    { icon: "🐦", label: "Dawn on X/Twitter", url: "https://x.com/dawninternet",                  color: "#e2e8f0" },
    { icon: "💬", label: "Dawn Discord",       url: "https://discord.gg/dawninternet",             color: "#5865f2" },
    { icon: "📱", label: "Dawn Telegram",      url: "https://t.me/dawninternet",                   color: "#2ba5e0" },
    { icon: "📖", label: "Dawn Docs",          url: "https://docs.dawninternet.com",               color: T.green },
    { icon: "🧪", label: "Dawn Research",      url: "https://www.dawninternet.com/research",       color: T.purple },
  ];

  return (
    <div style={css.screen}>
      <div style={{ padding: "1.25rem 1.25rem 2rem" }}>
        <button onClick={onClose} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: "0.85rem", fontFamily: "inherit", marginBottom: "1rem" }}>← Back</button>

        {/* Header */}
        <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
          <SunIcon size={64} glow />
          <div style={{ color: T.gold, fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.28em", marginTop: "1rem" }}>DAWNQUIZ</div>
          <div style={{ color: T.text, fontWeight: 900, fontSize: "1.5rem", marginTop: "0.3rem" }}>About This App</div>
          <div style={{ color: T.textMuted, fontSize: "0.82rem", marginTop: "0.4rem", lineHeight: 1.6, maxWidth: 300, margin: "0.4rem auto 0" }}>
            Version 1.0 · Community Project
          </div>
        </div>

        {/* What is DawnQuiz */}
        <div style={{ ...css.card, marginBottom: "0.85rem", borderLeft: `3px solid ${T.gold}` }}>
          <div style={{ color: T.gold, fontWeight: 800, fontSize: "0.82rem", marginBottom: "0.5rem" }}>☀ What is DawnQuiz?</div>
          <div style={{ color: T.text, fontSize: "0.85rem", lineHeight: 1.7 }}>
            DawnQuiz is an independent, community-built learning platform designed to help the Dawn Internet community
            deepen their knowledge of decentralized internet, DePIN, and the Dawn protocol through gamified quizzes,
            daily challenges, tournaments, and structured learning modules.
          </div>
        </div>

        {/* Who built it */}
        <div style={{ ...css.card, marginBottom: "0.85rem", borderLeft: `3px solid ${T.orange || "#FF6B1A"}` }}>
          <div style={{ color: T.orange || "#FF6B1A", fontWeight: 800, fontSize: "0.82rem", marginBottom: "0.5rem" }}>🛠 Who Built It?</div>
          <div style={{ color: T.text, fontSize: "0.85rem", lineHeight: 1.7 }}>
            DawnQuiz was built by a member of the Dawn community as a passion project to support ecosystem education
            and onboarding. It is not an official product of Dawn Internet or any affiliated entity.
          </div>
          <div style={{ marginTop: "0.75rem", padding: "0.5rem 0.75rem", background: T.surfaceHigh, borderRadius: 8 }}>
            <div style={{ color: T.textMuted, fontSize: "0.75rem", lineHeight: 1.6 }}>
              💡 Want to contribute questions, report bugs, or suggest features? Use the{" "}
              <span onClick={() => { onClose(); onNav("feedback"); }}
                style={{ color: T.gold, fontWeight: 700, cursor: "pointer" }}>
                Feedback
              </span>{" "}button on the home screen.
            </div>
          </div>
        </div>

        {/* What it covers */}
        <div style={{ ...css.card, marginBottom: "0.85rem" }}>
          <div style={{ color: T.text, fontWeight: 800, fontSize: "0.82rem", marginBottom: "0.6rem" }}>📚 What It Covers</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
            {[
              { icon: "🎯", text: "150+ questions across Beginner, Intermediate & Expert tiers" },
              { icon: "🌅", text: "Daily challenge questions with streak rewards" },
              { icon: "⚔",  text: "Community tournaments with live leaderboards" },
              { icon: "📘", text: "Dawn Academy — structured learning modules" },
              { icon: "◼",  text: "BlackBox Challenge & Deployer Final Run" },
              { icon: "🏆", text: "10-rank progression from Beginner to Deployer" },
            ].map(item => (
              <div key={item.text} style={{ display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
                <span style={{ flexShrink: 0, fontSize: "0.95rem" }}>{item.icon}</span>
                <span style={{ color: T.textMuted, fontSize: "0.8rem", lineHeight: 1.5 }}>{item.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Dawn official links */}
        <div style={{ ...css.card, marginBottom: "0.85rem" }}>
          <div style={{ color: T.text, fontWeight: 800, fontSize: "0.82rem", marginBottom: "0.75rem" }}>🔗 Official Dawn Resources</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.45rem" }}>
            {DAWN_LINKS.map(link => (
              <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer"
                style={{ display: "flex", alignItems: "center", gap: "0.65rem", padding: "0.6rem 0.75rem", background: T.surfaceHigh, borderRadius: 10, textDecoration: "none", border: `1px solid ${link.color}22` }}>
                <span style={{ fontSize: "1.1rem" }}>{link.icon}</span>
                <span style={{ color: link.color, fontWeight: 600, fontSize: "0.82rem", flex: 1 }}>{link.label}</span>
                <span style={{ color: T.textDim, fontSize: "0.75rem" }}>↗</span>
              </a>
            ))}
          </div>
        </div>
        ...
        {/* Disclaimer */}
        <div style={{ padding: "0.75rem 1rem", background: T.surface, borderRadius: 10, border: `1px solid ${T.border}`, textAlign: "center" }}>
          <div style={{ color: T.textDim, fontSize: "0.68rem", lineHeight: 1.7 }}>
            <span style={{ color: T.textMuted, fontWeight: 700 }}>Not affiliated with Dawn Internet.</span>
            {" "}This is an independent community project. It is not an official product, endorsement, or communication of Dawn Internet or any associated entity.
          </div>
        </div>
      </div>
    </div>
...
function Home({ player, onNav, onAdminClick, navLabels, setNavLabels, announcements = [], onShowWelcome, onAbout }) {
  const rank = getRank(player.sunrays);
  const next = getNextRank(player.sunrays);
  const prog = next ? ((player.sunrays - rank.threshold) / (next.threshold - rank.threshold)) * 100 : 100;
  ...
  const navItems = [
    ...
    { k: "challenge",  i: "🤝", label: "Challenge",    sub: "Dare a friend · Same questions", accent: "#f97316" },
    { k: "tournament", i: "⚔", label: "Tournaments", sub: "Compete · Leaderboard · Win", accent: "#8b5cf6" },
  ];
  ...
  return (
    <div style={css.screen}>
      {/* ── Hero header ── */}
      <div style={{
        background: `radial-gradient(ellipse at 50% 130%, #f97316 0%, #fbbf2444 30%, transparent 65%), linear-gradient(180deg, #050810 0%, #0d0e1e 30%, #1a0d02 60%, #2d1800 85%, #3d2000 100%)`,
        padding: "1.75rem 1.5rem 1.5rem", textAlign: "center",
        borderBottom: `1px solid #f9731622`, position: "relative",
      }}>
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginBottom: "1rem", position: "relative" }}>
          {onShowWelcome && (
            <button onClick={onShowWelcome} title="How to play" style={{ position: "absolute", left: 0, background: "none", border: `1px solid #f9731633`, borderRadius: 20, padding: "0.2rem 0.55rem", color: "#f97316aa", fontSize: "0.68rem", cursor: "pointer", fontFamily: "inherit" }}>Guide</button>
          )}
          <DawnLogo height={26} />
          {onAbout && (
            <button onClick={onAbout} title="About DawnQuiz" style={{ position: "absolute", right: 0, background: "none", border: `1px solid ${T.border}`, borderRadius: 20, padding: "0.2rem 0.55rem", color: T.textMuted, fontSize: "0.68rem", cursor: "pointer", fontFamily: "inherit" }}>About</button>
          )}
        </div>
        {player.username && (
          <div style={{ color: T.textMuted, fontSize: "0.72rem", marginBottom: "0.5rem" }}>
            Welcome back, <span style={{ color: T.text, fontWeight: 700 }}>{formatUsername(player.username)}</span> 👋
          </div>
        )}
        ...
        <div style={{ padding: "0.75rem 1.25rem 0" }}>
          {announcements.map(a => (
            <div key={a.id} style={{
              ...
              background: "linear-gradient(135deg,#0d1f10,#091a0d)",
              border: `1px solid ${T.green}44`, borderRadius: 12,
              padding: "0.75rem 1rem", marginBottom: "0.5rem",
            }}>
              <span style={{ fontSize: "1.1rem", flexShrink: 0 }}>📢</span>
              <div style={{ color: T.text, fontSize: "0.8rem", lineHeight: 1.55 }}>{a.text}</div>
            </div>
          ))}
        </div>
      )}
...
function Onboarding({ onComplete, viewOnly = false }) {
  ...
  const slides = viewOnly ? allSlides.filter(s => !s.isUsernameStep) : allSlides;
    ...
    <div style={{ ...css.screen, display: "flex", flexDirection: "column", justifyContent: "center", padding: "2rem 1.5rem" }}>
      ...
      <div style={{ marginTop: "auto", paddingTop: "2rem" }}>
        <Btn onClick={handleNext} style={{ background: slide.accent, border: "none", color: "#000", fontWeight: 800 }}>
          {isLast ? (viewOnly ? "Done ✓" : "Let's Go ☀") : "Next →"}
        </Btn>
        {step > 0 && (
...
export default function App() {
  ...
  useEffect(() => {
    logEvent("app_open", null, { url: window.location.href });
  }, []);
  const [showOnboarding, setShowOnboarding] = useState(false);

  // ── Announcements (admin-editable, localStorage) ────────────
  const [announcements, setAnnouncements] = useState(() => {
    try { return JSON.parse(localStorage.getItem("dawn_announcements") || "[]"); } catch { return []; }
  });
  function saveAnnouncements(list) {
    setAnnouncements(list);
    try { localStorage.setItem("dawn_announcements", JSON.stringify(list)); } catch {}
  }
  const activeAnnouncements = announcements.filter(a => a.active);
  const [tournamentCodeFromUrl, setTournamentCodeFromUrl] = useState(() => getTournamentCodeFromUrl());
  const [challengeCodeFromUrl,  setChallengeCodeFromUrl]  = useState(() => getChallengeCodeFromUrl());
  ...
  useEffect(() => {
    ...
    if (challengeCodeFromUrl  && player.onboarded) setScreen("challenge");
  ...
  const [adminInitialTab, setAdminInitialTab] = useState("questions");

  function handleAdminClick(tab = "questions") {
  ...
  const views = {
    home:        <Home player={player} onNav={setScreen} onAdminClick={handleAdminClick} navLabels={navLabels} setNavLabels={setNavLabels} announcements={activeAnnouncements} onShowWelcome={() => setShowOnboarding(true)} onAbout={() => setScreen("about")} />,
    about:       <AboutScreen onClose={() => setScreen("home")} onNav={setScreen} />,
    challenge:   <ChallengeScreen onExit={() => setScreen("home")} incomingCode={challengeCodeFromUrl} onCodeConsumed={() => setChallengeCodeFromUrl(null)} player={player} />,
    ...
    learn:       <LearningHub player={player} dispatch={dispatch} onExit={() => setScreen("home")} onAdmin={handleAdminClick} />,
    ...
    admin:       <AdminPanel player={player} dispatch={dispatch} onExit={() => { setScreen("home"); setCurrentAdmin(null); }} currentAdmin={currentAdmin} adminsHook={adminsHook} navLabels={navLabels} setNavLabels={setNavLabels} announcements={announcements} saveAnnouncements={saveAnnouncements} initialTab={adminInitialTab} />,
  };
      ...
      <>
        <style>{`
          ...
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          @keyframes fadeUp { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        `}</style>
        <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", position: "relative" }}>
          <button onClick={() => setShowOnboarding(false)} style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", color: "#9ca3af", fontSize: "1.2rem", cursor: "pointer", zIndex: 10 }}>✕</button>
          <Onboarding viewOnly onComplete={() => setShowOnboarding(false)} />
        </div>
      </>
    );
  }
