...
export async function statsTotalVisits() {
  const rows = await sb("GET", "app_events?event=eq.app_open&select=id") || [];
  return rows.length;
}

// Unique visitors (distinct usernames from app_open)
export async function statsUniqueVisitors() {
  const rows = await sb("GET", "app_events?event=eq.app_open&select=username") || [];
...
export async function statsTotalRuns() {
  const rows = await sb("GET", "app_events?event=eq.run_start&select=id") || [];
  return rows.length;
}
